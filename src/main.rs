...
fn key_expansion_core(state: &mut [u8; 4], round: usize) {
    state.rotate_left(1);
...
fn expand_key(key: &[u8; 16]) -> [u8; 176] {
    ...
    while bytes_generated < 176 {
        ...
        if bytes_generated % 16 == 0 {
            key_expansion_core(&mut temp, bytes_generated as usize / 16 - 1);
        }
...
fn encrypt(message: &String, key: &[u8; 16]) -> String {
    let num_rounds = 10;
    ...
    dbg!(state);
    dbg!(String::from_utf8(state.to_vec()));
    "Test".into()
}

fn main() {
    let text = "Hello, this is Nishan Rayamajhee".to_string();
